...
import { connectAuthEmulator, getAuth, User, onIdTokenChanged } from 'firebase/auth'
...
const initFirebase = () => {
  ...
  if (!import.meta.env.PROD) {
    ...
    console.log('development mode, using Emulator')
  ...
  // inject all function to excute when auth state changed
  setupOnAuthStateChanged(user, [userStore.handleAuthStateChanged], app)
...
export function setupOnAuthStateChanged(
  user: Ref<Nullable<User>>,
  excutedFunctions?: ((user: User) => void)[],
  app?: FirebaseApp
) {
  const auth = getAuth(app)
  ...
  onIdTokenChanged(auth, (userData) => {
    ...
    if (excutedFunctions !== undefined && userData !== null) {
      excutedFunctions.forEach((fn) => fn(userData))
    }
  })
}
