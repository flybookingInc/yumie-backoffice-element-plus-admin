...
import { makeOrderByAdminApi } from '@/api/order'

export const useReservationsStore = defineStore('app_reservationsStore', () => {
  ...
  let unsubscribeReservationData = () => {} // store reservation data unsubscribe function
  ...
  const decodeMessage = async (message: string): Promise<string> => {
    try {
      ...
      if (response.status === 200 && response.data && response.data.plainText) {
        return response.data.plainText as string
      }
      throw new Error(`解碼失敗`)
    } catch (e) {
      throw new Error(`解碼失敗: ${e}`)
    }
  }
  ...
  const getCustomerTags = async (hotelId: string, phoneNumber: string): Promise<string[]> => {
    ...
    if (response.status === 200 && response.data && response.data.tags) {
      console.log('Tags from server=', response.data.tags)
  ...
  const subscibeReservations = (hotelId: string, subscribeDate: Date) => {
    try {
      unsubscribeReservationData()
      reservationsTable.data = [] // reset reservationsTable.data
      const q = query(
        collection(getFirestore(), `qk-list/${hotelId}/orders`),
        where('check_in_date', '==', dayjs(subscribeDate).format('YYYY-MM-DD')),
        orderBy('check_in_date', 'asc')
      )
      unsubscribeReservationData = onSnapshot(
        q,
        (querySnapshot) => {
  ...
  const updateTagsCol = (tags: string[], rowId: string) => {
    console.log('enter updateTagsCol')
    for (let i = 0; i < reservationsTable.data.length; i++) {
      if (reservationsTable.data[i].id === rowId) {
        reservationsTable.data[i].qkTags = tags
        console.log('reservationsTable.data', reservationsTable.data[i])
        return
      }
    }
  }
  ...
  const getPhoneNumberByEncryedPhoneNumberOrCacheMap = async (
    ...
    if (!phonePlainText) {
      try {
        phonePlainText = await decodeMessage(encryptPhoneNumber)
        console.log('phonePlainText=', phonePlainText)
        phonePlainText = taiwanNationCodeFilter(phonePlainText)
        console.log('after filter phonePlainText=', phonePlainText)
  ...
  const updateReservationStatusColumn = async (
    ...
    console.log('updateReservationStatusColumn', hotelId, statusValue, reservationId)
  ...
  const makeOrderByAdmin = async (data: MakeOrderByAdminRequest) => {
    const response = await makeOrderByAdminApi(data)
